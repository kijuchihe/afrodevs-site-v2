...
const HireForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    fetch("https://formsubmit.co/ajax/build@afrodevs.com", {
      method: "POST",
      headers: {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="mb-2">
                <button
                  type="submit"
                  disabled={loading}
                  className={`block p-2 w-full text-white rounded-[4px] bg-green hover:bg-darkened-green ${loading ? "cursor-not-allowed" : "cursor-pointer"
                    }`}
                >
                  Submit
                </button>
